...
def detect(src, mask): #標識を検知する。検知した結果を書き込んだフレームと、赤い四角形の大きさ、標識までの距離を返す
    height, width, _ = src.shape
    contours, hierarchy = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    filtered_contour = []

    result = src.copy() #src.copy()に戻すこと
...
def putText(src, w, h, dist): #赤い四角形のサイズや標識までの距離を動画に入れる
    if (w < 0 or h < 0):
